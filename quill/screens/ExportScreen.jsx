...
export function logsToCSV(logs) {
  ...
  let rows = logs.map((l) => [(__propagate(l.dateISO).slice(0, 10) || ""), (l.category || "transport"), ((l.label || l.itemKey) || ""), (l.co2Kg || 0).toFixed(3), (l.quantity || 1), (l.notes || "")]);
...
export default function ExportScreen() {
  ...
  const deriveLogs = (appData) => {
    return (__propagate(appData).logs || []);
  };
  ...
  const deriveStats = (logList) => {
    ...
    let dates = new Set(logList.map((l) => __propagate(l.dateISO).slice(0, 10)));
