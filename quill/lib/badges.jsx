...
export let BADGES = [{ id: "first_log", name: "First Step", desc: "Log your first activity", icon: "🌱", check: (stats) => !(stats.totalLogs < 1) }, { id: "week_streak_3", name: "Consistent", desc: "3-day logging streak", icon: "🔥", check: (stats) => !(stats.currentStreak < 3) }, { id: "week_streak_7", name: "Full Week", desc: "7-day logging streak", icon: "⭐", check: (stats) => !(stats.currentStreak < 7) }, { id: "week_streak_14", name: "Fortnight Hero", desc: "14-day logging streak", icon: "🏅", check: (stats) => !(stats.currentStreak < 14) }, { id: "week_streak_30", name: "Monthly Master", desc: "30-day logging streak", icon: "🏆", check: (stats) => !(stats.currentStreak < 30) }, { id: "low_carbon_day", name: "Green Day", desc: "Score 80+ on any day", icon: "💚", check: (stats) => !(stats.bestDailyScore < 80) }, { id: "perfect_day", name: "Perfect Day", desc: "Score 100 on a day", icon: "🌟", check: (stats) => !(stats.bestDailyScore < 100) }, { id: "all_categories", name: "Well-Rounded", desc: "Log in all 4 categories in one day", icon: "🎯", check: (stats) => stats.hasLoggedAll4InOneDay }, { id: "fifty_logs", name: "Dedicated", desc: "Log 50 activities total", icon: "📊", check: (stats) => !(stats.totalLogs < 50) }, { id: "improvement", name: "Improver", desc: "Weekly score improved over last week", icon: "📈", check: (stats) => stats.weeklyImproved }];
export function computeStreak(logs) {
  ...
  return { currentStreak: current, longestStreak: Math.max(current, computeLongest(sorted)) };
}
export function computeLongest(sortedDatesDesc) {
